...
namespace sisVendas.Screens.Create
{
    ...
    {
		private ctrlProductCategory controlProductCategory = new ctrlProductCategory();
		...
		private bool isExistsProductCategory()
		{
			...
			if (name.Count() > 0)
			{
				DataTable dt = controlProductCategory.searthProductCategory(name);
				if (dt.Rows.Count > 0)
				{
					return true;
				}
		...
		{
			dgv_ProductCategory.DataSource = controlProductCategory.searthProductCategory(filtro);
		}
		...
		private void btnSave_Click(object sender, EventArgs e)
        {
			...
			else
			{
				...
				if (isOk)
				{
					if (controlProductCategory.SaveProductCategory(
						tbId.Text,
						tbName.Text
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
			...
			if (tbId.Text != "")
			{
				if (MessageBox.Show("Deseja excluir a categoria de produto ? ?", "Alerta!", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
				{
					...
					if (controlProductCategory.removeProduct(cod) == true)
					{
						updateDgv("");
